    ...
    <div className="h-64 w-full bg-paper p-6 rounded-[2rem] flex flex-col relative shadow-xl border border-dark/5 overflow-hidden">
      ...
      <div className="grid grid-cols-7 gap-1 sm:gap-2 mb-8 relative z-10">
        {days.map((d, i) => (
          <div 
            key={i} 
            className={`aspect-square rounded-md flex items-center justify-center font-data text-xs transition-colors duration-300 ${
              activeDay === i ? 'bg-accent text-paper shadow-lg scale-110' : 'bg-dark/5 text-dark/50'
            }`}
          >
            {d}
          </div>
